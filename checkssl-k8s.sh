...
echo -e "\nClient certificate:"
sudo grep "client-certificate" /etc/kubernetes/kubelet.conf | awk '{print $2}' | base64 -d | openssl x509 -noout -startdate -enddate -checkend 0 | sed "s/Certificate will expire/ (EXPIRED\!)/g" |  sed "s/Certificate will not expire/ (OK)/g"
echo "===="
